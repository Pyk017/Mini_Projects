from tkinter import *

root = Tk()
root.title("MY Calculatory")

intro = Label(root, text="Simple Calculator")
intro.grid(row=0, column=0, columnspan=4)

input_box = Entry(root, borderwidth=5, width=55)
input_box.grid(row=1, column=0, columnspan=4)

def button_click(number):
    global expression
    input_box.delete(0, END)
    expression += str(number)
    input_box.insert(0, expression) 
    

def button_clear():
    global expression
    expression = ''
    input_box.delete(0, END)

def button_equal():
    try:
        global expression
        result = str(eval(expression))
        input_box.delete(0, END)
        input_box.insert(0, result)
        expression = result
    except:
        pass

expression = ''

button_1 = Button(root, text="1", padx=40, pady=20, command=lambda : button_click(1))
button_2 = Button(root, text="2", padx=40, pady=20, command=lambda : button_click(2))
button_3 = Button(root, text="3", padx=40, pady=20, command=lambda : button_click(3))
button_minus = Button(root, text="-", padx=40, pady=20, command=lambda : button_click('-'))

button_4 = Button(root, text="4", padx=40, pady=20, command=lambda : button_click(4))
button_5 = Button(root, text="5", padx=40, pady=20, command=lambda : button_click(5))
button_6 = Button(root, text="6", padx=40, pady=20, command=lambda : button_click(6))
button_astrik = Button(root, text="*", padx=40, pady=20, command=lambda : button_click('*'))

button_7 = Button(root, text="7", padx=40, pady=20, command=lambda : button_click(7))
button_8 = Button(root, text="8", padx=40, pady=20, command=lambda : button_click(8))
button_9 = Button(root, text="9", padx=40, pady=20, command=lambda : button_click(9))
button_div = Button(root, text="/", padx=40, pady=20, command=lambda : button_click('/'))

button_0 = Button(root, text="0", padx=87, pady=20, command=lambda : button_click(0))
button_dot = Button(root, text=".", padx=40, pady=20, command=lambda : button_click('.'))
button_plus = Button(root, text="+", padx=40, pady=60, command=lambda : button_click('+'))
button_clear = Button(root, text="Clear", padx=125, pady=25, command=button_clear)


button_1.grid(row=4, column=0)
button_2.grid(row=4, column=1)
button_3.grid(row=4, column=2)
button_minus.grid(row=4, column=3)

button_4.grid(row=3, column=0)
button_5.grid(row=3, column=1)
button_6.grid(row=3, column=2)
button_astrik.grid(row=3, column=3)

button_7.grid(row=2, column=0)
button_8.grid(row=2, column=1)
button_9.grid(row=2, column=2)
button_div.grid(row=2, column=3)

button_0.grid(row=5, column=0, columnspan=2)
button_dot.grid(row=5, column=2)
button_plus.grid(row=5, column=3, rowspan=2)
button_clear.grid(row=6, column=0, columnspan=3)

button_equal = Button(root, text="=", padx=180, pady=20, command=button_equal)
button_equal.grid(row=7, column=0, columnspan=4)

root.mainloop()