	...
	def __init__(self):
		...
		for heightIdx in range(9):
			for widthIdx in range(9):
				boxIdx = int(widthIdx / 3) + 3 * int(heightIdx / 3)
				verticalLineIndex = widthIdx
				horizontalLineIndex = heightIdx + 9
				newCell = Cell(self.boxes[boxIdx], self.lines[verticalLineIndex], self.lines[horizontalLineIndex], (heightIdx, widthIdx))
				self.cells[heightIdx][widthIdx] = newCell
				self.boxes[boxIdx].cells.append(newCell)
	...
	def solve(self, values):
		...
		for valueToTry in range(1, 10):
			if self.isValid(valueToTry, firstUnsolvedAddress, values):
				...
				colorCell((125, 125, 125), firstUnsolvedAddress)
...
def checkInput():
	...
	for event in pygame.event.get():
		...
		if event.type == pygame.MOUSEBUTTONUP:
			...
			colorCell(green, selectedSpace)



def colorCell(withColor, cell):
	global offH
	global offW
	global highlight
	...
	highlightColor = withColor


def flashCell(cellNumber):
	for i in range(5):
		colorCell(red, cellNumber)
		drawBoard()
		pygame.time.wait(10)
		colorCell(white, cellNumber)
		drawBoard()
		pygame.time.wait(10)

	colorCell(green, cellNumber)
	drawBoard()
...
def drawBoard():
	...
	if highlight:
		pygame.draw.rect(screen, highlightColor, highlight)
	...
	for i in range(0, 10):
		...
		pygame.draw.line(screen, black, (offW + int(width / 9) * i, offH), (offW + int(width / 9) * i, offH + maxH), intensity)
		pygame.draw.line(screen, black, (offW, offH + int(height / 9) * i), (offW + maxW, offH + int(height / 9) * i), intensity)
...
with open('settings.txt') as settingFile:
...
pygame.display.set_caption('Sudoku Solver By Prakhar Kumar')
